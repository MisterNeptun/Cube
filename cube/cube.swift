...
func shuffle(cube:   RubiksCube)->RubiksCube{
    let shuffle_list: [(name: String, move: (RubiksCube) -> RubiksCube)] = [
        ("L_clockwise", L_clockwise),
        ("L_notclockwise", L_notclockwise),
        ("F_clockwise", F_clockwise),
        ("F_notclockwise", F_notclockwise),
        ("R_clockwise", R_clockwise),
        ("R_notclockwise", R_notclockwise),
        ("B_clockwise", B_clockwise),
        ("B_notclockwise", B_notclockwise),
        ("D_clockwise", D_clockwise),
        ("D_notclockwise", D_notclockwise),
        ("U_clockwise", U_clockwise),
        ("U_notclockwise", U_notclockwise)
    ]
    let original = init_cube()
    var result = cube
    for _ in 0..<UserDefaults.standard.integer(forKey: "shuffle") {
        if let moveTuple = shuffle_list.randomElement() {
            result = moveTuple.move(result)
            print("Move: \(moveTuple.name)")
            print(result)
        }
    }
    if result != original{
...
func R_clockwise(_ cube: RubiksCube)->RubiksCube{
    ...
    result.bottom [2][2] = original.back[0][0]
    result.bottom [1][2] = original.back[1][0]
    result.bottom [0][2] = original.back[2][0]
